...
def get_all_bundles(K, all_orders, rider, dist_mat):
    ...
    print(f"\nCalculating {rider.type} bundles ...")
    ...
    while(len(bundles[cur_depth]) != 0):
        ...
        for i in range(len(bundles[cur_depth-1])): # 여기서 bundle은, key 'seq'와 'candidate'로 이루어진 dictionary임
            ...
            for node in bundle['candidate']['ff']: # 여기서 node는, 이전 depth에서 앞, 앞에 붙을 수 있던 후보 node
                ...
                if (*shop_seq, *dlv_seq) in bundle_history :

                    if bundle_history[(*shop_seq, *dlv_seq)] == 0:
                        continue 
                    else:
                ...
                if test_route_feasibility_custom(all_orders, rider, shop_seq, dlv_seq) == 0:
                    ...
                    if sorted_shop_seq in comparison_dict:
                        # compare distance and save lowest sequence
                        total_dist = get_total_distance(dist_mat, shop_seq, dlv_seq, K)
                        if total_dist < comparison_dict[sorted_shop_seq]["cost"] :
                    ...
                    cand[seq_to_tuple]["ff"].add(node)
                    bundle_history[(*shop_seq, *dlv_seq)] = total_dist
                
                else :
                    bundle_history[(*shop_seq, *dlv_seq)] = 0
    ...
    print(f"\nReducing {rider.type} bundles ...")

    for key in bundles:
        bundle_group = {}
        for shop_seq, dlv_seq in bundles[key]:
            total_dist = sum(dist_mat[i,j] for (i,j) in zip(shop_seq[:-1], shop_seq[1:])) + dist_mat[shop_seq[-1],
                                                            dlv_seq[0]+K] + sum(dist_mat[i+K,j+K] for (i,j) in zip(dlv_seq[:-1], dlv_seq[1:]))

            bundle_family = tuple(sorted(shop_seq))

            if bundle_family in bundle_group:
                if bundle_group[bundle_family]['total_dist'] > total_dist:
                    bundle_group[bundle_family] = {
                        "seq" : (shop_seq, dlv_seq),
        ...
        bundles[key] = [bun['seq'] for _, bun in bundle_group.items()]
        print(f"[{rider.type} {key}] {len(bundles[key])} bundles ({time.time()-checkpoint:.4f}s)")
